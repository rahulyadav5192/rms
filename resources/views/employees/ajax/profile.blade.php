                ...
                <div class="row">
                    <div class="col-xl-7 col-md-6 mb-4 mb-lg-0">
                        ...
                        <x-cards.user :image="$employee->image_url">
                            ...
                            @if ($employee->status == 'active')
                                <p class="card-text f-12 text-lightest">
                                    <x-status :value="__('app.active')" color="light-green" />
                                </p>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                                ...
                                                <a class="dropdown-item openRightModal"
                                                    ...
                                                    href="{{ url('/account/employees/elcm', $employee->id) }}">@lang('ELCM')</a>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <x-cards.data :title="__('modules.client.profileInfo')" class=" mt-4">
                            ...
                            @if($employee->employeeDetail->bio_uid == '')
                            <x-cards.data-row :label="__('Biometric ID')"
                            :value="mb_ucwords($employee->employeeDetail->bio_uid) ?? '--'"  />
                            @else
                            <x-cards.data-row :label="__('Biometric ID')"
                            :value="mb_ucwords($employee->employeeDetail->bio_uid) ?? '--'" />
                            @endif
                            ...
                            @if ($showFullProfile)
                                ...
                                <x-cards.data-row :label="__('Aadhar No')"
                                :value="mb_ucwords($employee->employeeDetail->aadhar_no) ?? '--'" />
                                <x-cards.data-row :label="__('Pan No')"
                                :value="mb_ucwords($employee->employeeDetail->pan_no) ?? '--'" />
                                
                                @if($bankDetails)
                                    <x-cards.data-row :label="__('Bank Name')" 
                                        :value="mb_ucwords($bankDetails->bank_name ?? '--')" />
                                    <x-cards.data-row :label="__('Account Number')" 
                                        :value="$bankDetails->account_number ?? '--'" />
                                    <x-cards.data-row :label="__('IFSC Code')" 
                                        :value="$bankDetails->ifsc_code ?? '--'" />
                                    <x-cards.data-row :label="__('Branch')" 
                                        :value="mb_ucwords($bankDetails->branch_name ?? '--')" />
                                    <x-cards.data-row :label="__('Account Type')" 
                                        :value="mb_ucwords($bankDetails->account_type ?? '--')" />
                                @else
                                    <div class="text-muted">
                                        {{ __('No bank details found.') }}
                                    </div>
                                @endif
                                ...
                                @endif
                                <!--s {{$employee->employeeDetail->last_date}}-->
                                @if($employee->employeeDetail->last_date != '')
                                <a href="{{url('account/viewExp')}}/{{$employee->employeeDetail->user_id}}" target="_blank">
                                    <x-cards.data-row :label="__('Experience Letter')"
                                :value="'Click To View'" />
                                </a>
                                @endif
                                ...
                                <x-cards.data-row :label="__('Local Address')" 
    :value="($employee->employeeDetail->local_add ? mb_ucwords($employee->employeeDetail->local_add) . '<br>' : '') 
        . ($employee->employeeDetail->local_city ? mb_ucwords($employee->employeeDetail->local_city) . '<br>' : '') 
        . ($employee->employeeDetail->local_state ? mb_ucwords($employee->employeeDetail->local_state) . '<br>' : '') 
        . ($employee->employeeDetail->local_pin ? $employee->employeeDetail->local_pin : '') 
        ?: '--'" />
        <x-cards.data-row :label="__('Permanent Address')" 
    :value="($employee->employeeDetail->per_add ? mb_ucwords($employee->employeeDetail->per_add) . '<br>' : '') 
        . ($employee->employeeDetail->per_city ? mb_ucwords($employee->employeeDetail->per_city) . '<br>' : '') 
        . ($employee->employeeDetail->per_state ? mb_ucwords($employee->employeeDetail->per_state) . '<br>' : '') 
        . ($employee->employeeDetail->per_code ? $employee->employeeDetail->per_code : '') 
        ?: '--'" />
