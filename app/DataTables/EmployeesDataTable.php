...
class EmployeesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if ($this->editEmployeePermission == 'all'
                ...
                || ($this->editEmployeePermission == 'owned' && user()->id == $row->reporting_to)
                || ($this->editEmployeePermission == 'both' && (user()->id == $row->reporting_to || user()->id == $row->added_by))) {
    ...
    public function query(User $model)
    {
        ...
        $users = $model->with('role', 'roles', 'employeeDetail', 'session')
            ...
            ->select('users.id', 'employee_details.added_by', 'employee_details.reporting_to', 'users.name', 'users.email', 'users.created_at', 'roles.name as roleName', 'roles.id as roleId', 'users.image', 'users.status', DB::raw('(select user_roles.role_id from role_user as user_roles where user_roles.user_id = users.id ORDER BY user_roles.role_id DESC limit 1) as `current_role`'), DB::raw('(select roles.name from roles as roles where roles.id = current_role limit 1) as `current_role_name`'), 'designations.name as designation_name', 'branches.name as branch', 'teams.team_name as department', 'employee_details.employee_id', 'employee_details.joining_date')
        ...
        if ($request->branch != 'all' && $request->branch != '') {
            $users = $users->where('employee_details.branch_id', $request->branch);
        }
        ...
        if ($this->viewEmployeePermission == 'owned') {
            $users = $users->where('employee_details.reporting_to', user()->id);
        }

        if ($this->viewEmployeePermission == 'both') {
            $users = $users->where(function ($q) {
                $q->where('employee_details.reporting_to', user()->id);
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('modules.employees.branch') => ['data' => 'branch', 'name' => 'branch', 'title' => 'Branch'],
            __('modules.employees.department') => ['data' => 'department', 'name' => 'department', 'title' => 'Department'],
