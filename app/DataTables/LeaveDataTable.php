...
class LeaveDataTable extends BaseDataTable
{
    ...
    public function query(Leave $model)
    {
        ...
        ->selectRaw('leaves.*, leave_types.color, leave_types.type_name,
            ( select count("lvs.id") from leaves as lvs where lvs.unique_id = leaves.unique_id and lvs.duration = \'multiple\') as count_multiple_leaves')
        ->groupByRaw('ifnull(leaves.unique_id, leaves.id)');

        if ($this->viewLeavePermission == 'all') {
            $leavesList->where('leaves.manager_status', 0);
        }
        
        if (user()->username === 'NIF0725347') {
            $leavesList->where('employee_details.branch_id', 7);
        }
        // dd($leavesList->get());
