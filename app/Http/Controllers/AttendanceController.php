...
class AttendanceController extends AccountBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $if_tl = DB::table('teams')->where('d_head',user()->id)->pluck('id');
        ...
        if (request()->ajax()) {
            return $this->summaryData($request,$if_tl);
        }
        ...
        if(count($if_tl) > 0){
            $this->employees = User::join('employee_details', 'employee_details.user_id', '=', 'users.id')
                        ->select('users.*')
                        ->whereIn('employee_details.department_id', $if_tl)
                        ->get();
        }
    ...
    public function summaryData($request,$if_tl)
    {
        ...
        $employees = User::with(
            ...
            ->select('users.id', 'users.name', 'users.email','users.status', 'users.created_at', 'employee_details.department_id', 'users.image')
            ...
            //     if(count($if_tl) > 0){
            //         $employees->whereIn('employee_details.department_id', $if_tl);
            //     }
            // }
        ...
        if (auth()->user()->id == 16531) {
            $employees = $employees->whereIn('employee_details.department_id', [20]);
        }
        ...
        //     if(count($if_tl) <1){
        //         if ($this->viewAttendancePermission == 'owned') {
        //             $employees = $employees->whereIn('users.id', user()->id);
        //         }
        //     }
        // }
    ...
    public function exportAllAttendance($year, $month, $id, $late, $department, $designation,$name,$branch)
    {
        $startDate = Carbon::createFromFormat('d-m-Y', '01-' . $month . '-' . $year)->startOfMonth()->startOfDay();
        $endDate = $startDate->copy()->endOfMonth()->endOfDay();
        ...
        return Excel::download(new AttendanceExport($year, $month, $id, $late, $department, $designation, $startDate, $endDate,$name,$branch), 'Attendance_From_' . $startDate->format('d-m-Y') . '_To_' . $date->format('d-m-Y') . '.xlsx');
    }
    ...
    public function attendence(){
        ...
        foreach($employee as $emp){
            ...
            $response = Http::post('https://sky.udtahathi.xyz/api/get_checkin', $data);
...
// return $late;
    ...
    public function attendence_prev(){
        
          ini_set('max_execution_time', 15000);
        //  dispatch(new attendence_prev())->timeout(15 * 60);
        // current date 
        // $todayDate = now()->toDateString();
        $todayDate = Carbon::now()->subDay(8)->format('Y-m-d');
        // return $todayDate;
        $employee = DB::table('employee_shift_schedules')->whereDate('employee_shift_schedules.date', $todayDate)
                    ->join('employee_details','employee_details.user_id','employee_shift_schedules.user_id')
                    ->join('bio_machine','bio_machine.id','employee_details.bio_machine_id')
                    ...
                    ->whereNotNull('employee_details.bio_machine_id')
                    ->whereNotNull('employee_details.bio_uid')
                    ->where('employee_details.user_id',2782)
                    ->where('bio_machine.status',0)
                    ->get();
        // return $employee;
        $date = $todayDate;
...
// return $late;
    ...
    public function storage(Request $request){
        ...
        $update = DB::table('user')->where('user_id',$request->user_id)->update($data);
        if($update){
            return $request->all()->input();
        }else{
    ...
    public function cron_job_function(Request $request){
        
        $attendanceResult = $this->attendence($request);
        $preattendanceResult = $this->attendence_prev($request);

    }
    
    
    public function att_dis(Request $request){
        $this->dispute = DB::table('att_despute')
                    ->join('employee_details', 'att_despute.user_id', '=', 'employee_details.user_id')
                    ->join('users', 'users.id', '=', 'employee_details.user_id')
                    
                    ->where('att_despute.user_id',user()->id)
                    ->latest('att_despute.id')
                    ->select('att_despute.*','users.name','users.username')
                    ->get();
        return view('att_dis',$this->data);
        
    }
    
    public function att_dis_save(Request $request){
        
        $request->validate([
            'date' => 'required',
            'log_in_time' => 'required',
            'out_date' => 'required',
            'log_out_time' => 'required',
            'des' => 'required',
        ]);
        
        $save = DB::table('att_despute')->insert([
                'user_id' => user()->id,
                'date' => $request->date,
                'log_in_time' => $request->log_in_time,
                'logout_date' => $request->out_date,
                'log_out_time' => $request->log_out_time,
                'desc' => $request->des,
                
            ]);
        return redirect('/att_dis')->with('message','Your Dispute Has Been Raised Successfully');
        
        
    }
    
    public function att_dis_view($id){
        $this->pageTitle = 'Attendance Dispute';
        $team = DB::table('teams')->where('id',$id)->where('d_head',user()->id)->first();
        $this->dispute = DB::table('att_despute')
                    ->join('employee_details', 'att_despute.user_id', '=', 'employee_details.user_id')
                    ->join('users', 'users.id', '=', 'employee_details.user_id')
                    ->join('teams', 'employee_details.department_id', '=', 'teams.id')
                    ->where('teams.id', $team->id)
                    // ->where('att_despute.solve_status','!=', 1)
                    ->latest('att_despute.id')
                    ->select('att_despute.*','users.name','users.username')
                    ->get();
        // dd($tickets);
        return view('att_des_view_head',$this->data);
        
    }
    
    public function att_dis_view_hr(){
        // if(user()->id != 1697){
        //     return 'you are not eligible to view page';
        // }
        $this->pageTitle = 'Attendance Dispute';
        $this->dispute = DB::table('att_despute')
                    ->join('employee_details', 'att_despute.user_id', '=', 'employee_details.user_id')
                    ...
                    ->get();
        // dd($tickets);
        return view('att_dis_hr',$this->data);
        
    }
    
    public function att_dis_approve(Request $request, $user_id,$id){
        $request->validate([
            'date' => 'required',
            'log_in_time' => 'required',
        ...
        // $date = '2023-11-30'; // Replace with your actual date
        // $time = '10:16:31';   // Replace with your actual time
        
        $log_in = $request->date . ' ' . $request->log_in_time;
        $log_out = $request->logout_date . ' ' . $request->log_out_time;
        
        // $update = DB::table('attendances')->insert([
        //                 'user_id' => $user_id,
        //                 'company_id' =>1,
        //                 'location_id' =>1,
        //                 'clock_in_time' => $log_in,
        //                 'clock_out_time' => $log_out,
        //                 'late' => $request->late,
        //                 'work_from_type'=>'office',
        //                 'half_day'=>$request->half_day,
        //             ]);
        
        $save = DB::table('att_despute')->where('id',$id)->update([
                // 'user_id' => user()->id,
                'date' => $request->date,
                'log_in_time' => $request->log_in_time,
                'log_out_time' => $request->log_out_time,
        ...
        $update = DB::table('att_despute')->where('id',$id)->update([
                    'manager_status' => 2
            ]);
        return redirect()->back()->with('message','Dispute Approved Successfully! Sent To HR');
    }
    
    public function att_dis_approve_hr(Request $request, $user_id,$id){
        $request->validate([
            'date' => 'required',
            'log_in_time' => 'required',
            'log_out_time' => 'required',
        ...
        $log_in = $request->date . ' ' . $request->log_in_time;
        $log_out = $request->date . ' ' . $request->log_out_time;
        
        $update = DB::table('attendances')
        ->updateOrInsert(
            [
                'user_id' => $user_id,
                'date' => date('Y-m-d', strtotime($request->date)), // Adjust 'date_column_name' with the actual date column name
            ],
            [
                'company_id' => 1,
                'location_id' => 1,
        ...
        return redirect()->back()->with('message','Dispute Approved Successfully');
    }
    
    public function reject_att_dis($id){
        $update = DB::table('att_despute')->where('id',$id)->update([
                    'manager_status' => 1
            ]);
        return redirect()->back()->with('message','Dispute Rejected Successfully');
    }
    public function reject_att_dis_hr($id){
        $update = DB::table('att_despute')->where('id',$id)->update([
                    'solve_status' => 1
            ]);
        return redirect()->back()->with('message','Dispute Rejected Successfully');
    ...
    // get the shift data from excel 
    public function excel_Shift(){
        $filePath = url('public/text_shift.xlsx');
