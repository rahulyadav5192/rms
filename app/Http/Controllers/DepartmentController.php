...
class DepartmentController extends AccountBaseController
{
    ...
    public function my_team($id){
         $query = DB::table('users');
            $query->join('employee_details','employee_details.user_id','users.id')->where('employee_details.department_id',$id);
            ...
            $perPage = 15; // Number of employees to display per page
            ...
            $this->roles = Role::where('name', '<>', 'client')
                ->orderBy('id')->get();
        return view('departments.d_head_view',$this->data);
                // return $dataTable->render('departments.d_head_view', $this->data);
    }
    ...
    public function update(UpdateDepartment $request, $id)
    {
        ...
        $group->d_head = $request->department_head ?? null;
        $group->save();
