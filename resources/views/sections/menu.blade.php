...
if (!in_array('admin', user_roles()) && $userId != 692) {
    ...
    // Check if form is filled (hide if NOT filled)
    $form = DB::table('users')->select('form_filled')->where('id', $userId)->first();
    $formFilled = $form?->form_filled ?? 0;  // Default to 0 if null

    // Check for unaccepted policies (fixed table name)
    $unacceptedPolicyCount = DB::table('policies as p')
        ->leftJoin('pilicy_accept as pa', function($join) use ($userId) {
            $join->on('p.id', '=', 'pa.policy_id')
                ->where('pa.user_id', $userId)
                ->where('pa.trash_status', 0);
        })
        ->where('p.trash_status', 0)
        ->whereNull('pa.id') // Not accepted
        ->count();
    ...
    $uploadedDocs = DB::table('employee_docs')
    ...
    // Final condition: Hide if form NOT filled OR unaccepted policies OR missing docs
    if ($formFilled != 0 || $unacceptedPolicyCount > 0 || !empty($missingDocs)) {
        $con = 'not';
    }

    // TEMP DEBUG: Log to check values (view in storage/logs/laravel.log)
    \Log::info("Menu Check for User {$userId}: formFilled={$formFilled}, unaccepted={$unacceptedPolicyCount}, missing=" . json_encode($missingDocs));
$debugMsg = "Menu Check for User {$userId}: formFilled={$formFilled}, unaccepted={$unacceptedPolicyCount}, missing=" . json_encode($missingDocs) . ", con=" . json_encode($con);
    echo "<script>console.log(" . json_encode($debugMsg) . ");</script>";
}
@endphp
...
@if($con=='yes')
    @if (!in_array('client', user_roles()) && (in_array('employees', user_modules()) || in_array('leaves', user_modules()) || in_array('attendance', user_modules()) || in_array('holidays', user_modules())) && ($sidebarUserPermissions['view_employees'] != 5 || $sidebarUserPermissions['view_leave'] != 5 || $sidebarUserPermissions['view_attendance'] != 5 || $sidebarUserPermissions['view_holiday'] != 5) && ($sidebarUserPermissions['view_employees'] != 'none' || $sidebarUserPermissions['view_leave'] != 'none' || $sidebarUserPermissions['view_attendance'] != 'none' || $sidebarUserPermissions['view_holiday'] != 'none' || $sidebarUserPermissions['view_shift_roster'] != 'none'))
        <x-menu-item icon="people" :text="__('app.menu.hr')">
            ...
            <div class="accordionItemContent pb-2">
                ...
                @endif
                @php
                 $d_head = DB::table('teams')->where('d_head',user()->id)->get();
                @endphp
                @if(count($d_head) != 0)
                    @foreach($d_head as $d)
                        @php
                            $teamUrl = route('departments.d_head_view', ['id' => $d->id]);
                            $teamText = __($d->team_name);
                    
                            $teamDisput = route('departments.disput', ['id' => $d->id]);
                            $teamDisputUrl = __($d->team_name . ' Dispute');
                    
                            $managerApprovalUrl = route('manager.travel.requests', ['team_id' => $d->id]);
                            $managerApprovalText = __($d->team_name . ' Travel Requests');
                        @endphp
                    
                        <x-sub-menu-item :link="$teamUrl" :text="$teamText" />
                        <x-sub-menu-item :link="$teamDisput" :text="$teamDisputUrl" />
                        <x-sub-menu-item :link="$managerApprovalUrl" :text="$managerApprovalText" />
                    @endforeach
                    ...
                    @foreach($d_head as $d)
                         @php
                            $teamUrl = route('head_leave_manage', ['team_id' => $d->id]);
                            $teamText = __($d->team_name.' Leaves');
                ...
                @endif
                @php
                    $allowedUsers = ['NIF0725407', 'NIF0525003', 'NIF0725347','NIFALPHA'];
                @endphp
                
                @if(in_array(auth()->user()->username, $allowedUsers))
                    <!--<a href="{{ route('kolkatta.team.leave.manage') }}" class="btn btn-primary">-->
                    <!--    Manage Team Leaves (Kolkata)-->
                    <!--</a>-->
                    
                    <x-sub-menu-item :link="url('kolkatta-team-leaves')" :text="__('Kolkata Leaves')" />
                @endif
                ...
                <x-sub-menu-item :link="url('/account/settings/profile-settings?tab=documents')" :text="__('Documents')" />
                <x-sub-menu-item :link="url('/policies')" :text="__('Policies')" />
                <x-sub-menu-item :link="url('/att_dis')" :text="__('Attendance Regularization')" />
                <x-sub-menu-item :link="url('/account/myletters')" :text="__('Letters')" />
                <x-sub-menu-item :link="url('/travel-allowance')" :text="__('Travel Allowance')" />
                @if(user()->permission('edit_employees') == 'all')
                <x-sub-menu-item :link="url('/admin/travel-requests')" :text="__('Travel Allowance Requests')" />
                <x-sub-menu-item :link="url('/account/employees/erp')" :text="__('Referrals')" />
                @endif
                <x-sub-menu-item :link="url('/account/employees/my_erp')" :text="__('My References')" />
                @if(user()->id == 1697)
                <x-sub-menu-item :link="url('/att_dis_view_hr')" :text="__('All Attendence Disputes')" /> 
                @endif
                @if(in_array(auth()->user()->username, ['NIF1222001','NIF0223001']))
                <x-sub-menu-item :link="url('/account/offerLetter')" :text="__('Generate Offer Letter')" />
                @endif
                <x-sub-menu-item :link="url('account/skills/employee')" :text="__('Skills Tracker')" />
                <x-sub-menu-item :link="url('joining_form')" :text="__('Joining Form')" />
                <x-sub-menu-item :link="url('knowledge-base')" :text="__('Knowledge Base')" />
                <x-sub-menu-item :link="url('complaints')" :text="__('Complaints')" />
                ...
                @endif
                @if(in_array(auth()->user()->username, ['NIFALPHA','NIF1022003','NIF1122045','NIF0425246','NIF1019001']))
    ...
    @if(auth()->user()->id == 1)
    <x-menu-item icon="house" :text="__('Skills Tracker')">
            <x-slot name="iconPath">
                <path fill-rule="evenodd"
                      d="M2 13.5V7h1v6.5a.5.5 0 0 0 .5.5h9a.5.5 0 0 0 .5-.5V7h1v6.5a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 2 13.5zm11-11V6l-2-2V2.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5z" />
                <path fill-rule="evenodd"
            ...
            <div class="accordionItemContent pb-2">
                <x-sub-menu-item :link="route('skills.index')"
                                 :text="__('Tasks')" />
                <x-sub-menu-item :link="route('skills.create')"
                                 :text="__('Add Tasks')" />
            </div>
        </x-menu-item>
    @endif
