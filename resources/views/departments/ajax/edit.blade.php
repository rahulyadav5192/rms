        ...
        <x-form id="save-department-data-form" method="put">
            <div class="add-client bg-white rounded">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-6">
                        <x-forms.label class="my-3 mt-2" fieldId="department_head" :fieldLabel="__('Department Head')" fieldName="department_head">
                        </x-forms.label>
                        <x-forms.input-group>
                            <select class="form-control select-picker mt" name="department_head" id="department_head" data-live-search="true">
                                <option value="">-- Select Department Head --</option>
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" @if($department->d_head == $employee->id) selected @endif>{{ $employee->name }}</option>
                                @endforeach
                            </select>
                        </x-forms.input-group>
                    </div>
