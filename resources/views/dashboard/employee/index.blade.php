...
@section('content')
            ...
            @if(count(array_intersect(['profile', 'shift_schedule', 'birthday', 'notices'], $activeWidgets)) > 0)
                <div class="col-xl-5 col-lg-12 col-md-12 e-d-info-notices">
                    <div class="row">
                        ...
                        @if(user()->niftel_rate == NULL)
                        <div class="col-md-12" id="rating-div">
                            <div class="card border-0 b-shadow-4 mb-3 e-d-info">
                                <div class="card-horizontal align-items-center">
                                    <div class="card-img">
                                        <img class=""  style="object-fit: contain !important;" src="https://niftel.com/logo-old.png" alt="Niftel image">
                                    </div>
                                    <div class="card-body border-0 pl-0" id="whole-rate">
                                        <h4 class="card-title f-18 f-w-500 mb-2">Rate Niftel </h4>
                                    ...
                                    <div style="margin-right: 23px;"  id="submitBtnContainer">
                                        <button class="buttonn" id="submitButton">Submit</button>
                                    </div> 
                                </div> 
                            </div>
                        </div>
                        @endif
    <!--                    <button id="viewPdfBtn" style="margin: 20px; padding: 10px 20px;background-color: var(--header_color) !important;-->
    <!--border: 1px solid var(--header_color) !important;">View Employee Handbook</button>-->
    ...
    <div id="pdfModal">
        <div id="viewerControls">
            ...
            <span>Page <input type="number" id="pageNum" min="1" value="1" /> / <span id="pageCount">--</span></span>
            <button id="nextPage">Next ⟩</button>
        </div>
        <button id="closeBtn">Close</button>
        <div id="watermark">NIFTEL</div>
        <canvas id="pdfCanvas"></canvas>
    </div>

    <script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.4.120/pdf.min.js"></script>
    <script>
        const openBtn = document.getElementById('viewPdfBtn');
    ...
    pdfDoc.getPage(num).then(function (page) {
        const modal = document.getElementById('pdfModal');
        const availableWidth = modal.clientWidth;
        const availableHeight = modal.clientHeight - 60; // for controls
        ...
        const renderContext = {
            canvasContext: ctx,
            viewport: viewport,
            transform: [pixelRatio, 0, 0, pixelRatio, 0, 0] // scale context for high DPI
        };
