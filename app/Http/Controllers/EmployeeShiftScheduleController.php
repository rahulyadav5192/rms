...
class EmployeeShiftScheduleController extends AccountBaseController
{
    ...
    public function import_shift(Request $request)
    {
        $this->viewShiftPermission = user()->permission('view_shift_roster');
        $this->manageEmployeeShifts = user()->permission('manage_employee_shifts');
        ...
        return view('shift-rosters.import',$this->data);

    }
    
    public function import_shift_save(Request $request)
    {
        $this->viewShiftPermission = user()->permission('view_shift_roster');
        $this->manageEmployeeShifts = user()->permission('manage_employee_shifts');
    
        abort_403(!(in_array($this->viewShiftPermission, ['all'])));
        ...
        // Define the starting index for the days of the week columns
        $startDayIndex = 22;
        ...
        foreach ($dataArray as $key => $row) {
            ...
            try {
                $empId = $row[1]; // Assuming 'EMP ID' is the second column (index 1)
    
                // Retrieve user_id from employee_detail table
                $user = DB::table('employee_details')->where('employee_id', $empId)->first();
    
                if (!$user) {
                    throw new \Exception("Employee with EMP ID $empId not found.");
                }
    
                $userId = $user->user_id;
    
                foreach ($daysOfWeek as $dayIndex => $day) {
                    $dateColumnIndex = $startDayIndex + $dayIndex;
                    $headerDate = $header[$dateColumnIndex];
    
                    // Extract the date part from the header
                    $date = Carbon::createFromFormat('l, F d, Y', $headerDate)->format('Y-m-d');
                    $shiftTime = $row[$dateColumnIndex];
                    
                    if($shiftTime == '-' or $shiftTime == ' - '){
                        // throw new \Exception("Employee with EMP ID $empId and date $date. Data not found...");
                        continue;
                    }
                    
                    if($shiftTime == 'WO' or $shiftTime == 'wo'){
                        $shiftTime = 1;
                    }else{
                        $shiftTime = (int)$row[$dateColumnIndex];
                    }
                    
                    if($shiftTime == 'Leave' or $shiftTime == 'leave'){
                        continue;
                    }
    
                    // Collecting data to see if it's parsed correctly
                    $data[] = [
                        'user_id' => $userId,
                        'date' => $date,
                    ...
                    // Save to the database using query builder
                    // if($userId == 3000){
                        DB::table('employee_shift_schedules')->updateOrInsert(
                            ['user_id' => $userId, 'date' => $date],
                            ['employee_shift_id' => $shiftTime,'last_updated_by' => user()->id,'added_by' => user()->id]
                        );
                    // }
                    
                }
            } catch (\Exception $e) {
                $errors[] = "Error on row {$key}: " . $e->getMessage();
            }
        }
    
        // For debugging purposes, returning the data
        // dd($data); // Uncomment this line to see the data in your browser
        ...
        return back()->with('message', 'Shift data imported successfully.');
    }
