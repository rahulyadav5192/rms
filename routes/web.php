...
use App\Http\Controllers\ElcmController;
...
use App\Http\Controllers\AccountsAttendance;
...
use App\Http\Controllers\SalarySheetUpload;
...
use App\Http\Controllers\ComplaintController;
    ...
    Route::get('account/employees/excel', [EmployeeController::class, 'export'])->name('export-employees');
    // Route::get('all_photo', [EmployeeController::class, 'all_photo'])->name('export-all_photo');
    
    Route::get('/employees/bulk-update', [EmployeeController::class, 'bulk_update']);
    Route::post('/employees/bulk-update', [EmployeeController::class, 'store_bulk_update']);


    Route::get('new_ol', function(){
        return view('letter.new_ol');
    });
    
    
    Route::get('/letter/signature', function () {
        return view('letter.sign');
    });
    
    Route::group(['middleware' => ['auth']], function () {
        Route::get('/knowledge-base', [EmployeeController::class, 'knowledge'])->name('knowledge.index');
    });
    
    Route::post('/signature-upload', [SignatureController::class, 'upload'])->name('signature.save');
    ...
    Route::middleware(['auth'])->group(function () {
        ...
        Route::post('/complaint_save', [ComplaintController::class, 'store'])->name('complaints.store');
        Route::get('/complaints/manage', [ComplaintController::class, 'manage'])->name('complaints.manage');
        Route::post('/complaints/{id}/status', [ComplaintController::class, 'updateStatus'])->name('complaints.updateStatus');
    });


    Route::group(['middleware' => ['auth']], function () {
        Route::get('/travel-allowance', [TravelController::class, 'index'])->name('travel-allowance.index');
        Route::post('/travel-allowance', [TravelController::class, 'store'])->name('travel-allowance.store');
        Route::get('/travel-allowance/{id}/edit', [TravelController::class, 'edit'])->name('travel-allowance.edit');
        Route::put('/travel-allowance/{id}', [TravelController::class, 'update'])->name('travel-allowance.update');
        Route::delete('/travel-allowance/{id}', [TravelController::class, 'destroy'])->name('travel-allowance.destroy');
        Route::get('/travel/{id}/details', [TravelController::class, 'viewDetails'])->name('travel.details');
        
        Route::get('/accounts/travel-requests', [TravelController::class, 'travelRequestsList'])->name('accounts.travel.list');
        Route::get('/accounts/travel-requests/{travel_id}/details', [TravelController::class, 'accountsTravelDetails'])->name('accounts.travel.details');
        Route::post('/accounts/travel-requests/{id}/update-status', [TravelController::class, 'changeTravelRequestStatus'])->name('accounts.travel.updateStatus');
        Route::post('/accounts/travel-requests/{id}/update-invoice-status', [TravelController::class, 'updateInvoiceAccountsStatus'])->name('accounts.travel.update.invoice.status');
        // Other routes...
    
        // manager 
        Route::get('/manager/travel-requests/{team_id}', [TravelController::class, 'managerApprovalPage'])->name('manager.travel.requests');
        Route::get('/manager/travel-requests/{team_id}/{travel_id}/details', [TravelController::class, 'managerTravelDetails'])->name('manager.travel.details');
        Route::post('/manager/travel-requests/{id}/update-status', [TravelController::class, 'updateManagerStatus'])->name('manager.travel.update.status');
        Route::post('/manager/travel-requests/{id}/update-invoice-status', [TravelController::class, 'updateInvoiceManagerStatus'])->name('manager.travel.update.invoice.status');
        
        // invoice employee
    
        Route::post('/employee/upload-invoices/{id}', [TravelController::class, 'uploadInvoices'])->name('employee.uploadInvoices');
        Route::delete('/employee/delete-invoice/{travelId}/{invoiceId}', [TravelController::class, 'deleteInvoice'])->name('employee.deleteInvoice');
        
    });
    
    Route::group(['middleware' => 'auth', 'prefix' => '/accounts/attendance'], function () {
        
        // routes for attendance show for accounts team 
        // Route::get('/admin', [TravelController::class, 'index'])->name('attendance.admin');
        Route::get('agent/{userid}', [AccountsAttendance::class, 'agent_dash']);
        Route::get('non-csa/{userid}', [AccountsAttendance::class, 'non_csa_dash']);
    ...
    Route::group(['middleware' => 'auth', 'prefix' => '/agent'], function () {
        Route::get('/attendance/upload', [AgentAttendanceController::class, 'upload'])->name('agent.upload');
        Route::post('/upload-attendance', [AgentAttendanceController::class, 'uploadAttendance'])->name('upload.attendance');
        Route::post('/upload/non-csa-attendance', [AgentAttendanceController::class, 'uploadNonCsaAttendance'])->name('upload.non.csa.attendance');
    });


    Route::get('attendences/zk', [AttendanceController::class, 'attendence']);
    Route::get('attendences/zk/pre', [AttendanceController::class, 'attendence_prev']);
    
    
    Route::get('machine_emp', [LiveController::class, 'machine_emp']);
    
    
    // Leave Manager 
    Route::get('head_leave_manage/{team_id}', [LeaveController::class, 'head_leave_manage'])->name('head_leave_manage')->middleware('auth');;
    Route::get('manger_approve_leave/{id}', [LeaveController::class, 'manger_approve_leave'])->middleware('auth');;
    Route::get('manger_reject_leave/{id}', [LeaveController::class, 'manger_reject_leave'])->middleware('auth');;
    Route::get('/kolkatta-team-leaves', [LeaveController::class, 'kolkatta_team_leave_manage'])
    ->name('kolkatta.team.leave.manage')
    ->middleware('auth');
    Route::get('excel_Shift', [AttendanceController::class, 'excel_Shift']);
    // Route::middleware([Policy::class])->group(function () {
    ...
    Route::get('deactive', [LiveController::class, 'deactive']);
    Route::get('updateWorkFromHome', [LiveController::class, 'updateWorkFromHome']);
    Route::get('inactive_us', [LiveController::class, 'inactive_us']);
    
    
    Route::get('cron_job_function', [LiveController::class, 'cron_job_function']);
    Route::get('cron_job_function_machine', [LiveController::class, 'cron_job_function_machine']);
    Route::get('all_machine', [LiveController::class, 'all_machine']);
    Route::get('deactive_machine/{ip}', [LiveController::class, 'deactive_machine']);
    Route::get('active_machine/{ip}', [LiveController::class, 'active_machine']);
    Route::get('flexi', [LiveController::class, 'flexi']);
    Route::get('flexi_pre', [LiveController::class, 'flexi_pre']);
    Route::get('export_csv_new', [LiveController::class, 'export_csv'])->middleware(['auth',]);
    Route::get('export_inactive', [LiveController::class, 'export_inactive']);
    Route::get('attendence_shift', [LiveController::class, 'attendence_shift']);

// joining form 

Route::group(['middleware' => 'auth',], function () {
    
    Route::get('/policies', [EmployeeController::class, 'policies']); 
    Route::get('/police_accept', [EmployeeController::class, 'police_accept']);
    Route::get('/policy_accept/{id}', [EmployeeController::class, 'policy_accept_user']);
    Route::get('/policy_accept_all', [EmployeeController::class, 'policy_accept_all']);
    
    Route::get('/police_create', [EmployeeController::class, 'police_create']);
    
    Route::post('/policy_change/{id}', [EmployeeController::class, 'policy_change']);
    Route::post('/policy_add', [EmployeeController::class, 'policy_add']);
    
    Route::get('/policy_delete/{id}', [EmployeeController::class, 'policy_delete']);

    
    Route::get('joining_form', [EmployeeController::class, 'joining_form']);
    Route::post('submit_join_form', [EmployeeController::class, 'submit_join_form']);
    
    
    Route::get('att_dis', [AttendanceController::class, 'att_dis']);
    Route::post('att_dis_save', [AttendanceController::class, 'att_dis_save']);
    
    Route::get('att_dis_view/{id}', [AttendanceController::class, 'att_dis_view'])->name('departments.disput');
    Route::post('att_dis_approve/{user_id}/{id}', [AttendanceController::class, 'att_dis_approve']);
    Route::get('reject_att_dis/{id}', [AttendanceController::class, 'reject_att_dis']);
    
    
    // att dis hr 
    Route::get('att_dis_view_hr', [AttendanceController::class, 'att_dis_view_hr'])->name('departments.disput.hr');
    Route::post('att_dis_approve_hr/{user_id}/{id}', [AttendanceController::class, 'att_dis_approve_hr']);
    Route::get('reject_att_dis_hr/{id}', [AttendanceController::class, 'reject_att_dis_hr']);
    
    
    Route::get('/spin', [SpinController::class, 'spin']);
    
    Route::get('/rating/{rate}', [LiveController::class, 'rating']);
    
    
    Route::get('/account/employees/elcm/{user_id}', [ElcmController::class, 'elcm']);
    Route::get('/account/employees/elcm', [ElcmController::class, 'elcm_red']);
    Route::post('/account/employees/elcm/elcm_edit/{user_id}', [ElcmController::class, 'elcm_edit']);
    Route::post('/account/employees/elcm/uploadCsv', [ElcmController::class, 'uploadCsv']);
  ...
  Route::group(['middleware' => 'auth', 'prefix' => '/account/employees'], function () {
      
    // References manage
    Route::get('/erp', [ErpController::class, 'erp_index']);
    Route::post('/erp_add', [ErpController::class, 'store']);
    Route::post('/erp_update/{id}', [ErpController::class, 'erp_update']);
    Route::get('/erp_delete/{id}', [ErpController::class, 'erp_delete']);
    Route::post('/erp_upload', [ErpController::class, 'uploadCsv']);
    
    // My  References 
    Route::get('/my_erp', [ErpController::class, 'my_erp_index']);
    Route::post('/my_erp_add', [ErpController::class, 'my_store']);
    // Route::get('/erp_delete/{id}', [ErpController::class, 'erp_delete']);
  }); 
    ...
    Route::get('salary', [SalaryController::class, 'main']);
    Route::get('salary_slip', [SalaryController::class, 'salary_slip']);
    Route::get('salary_detail', [SalaryController::class, 'salary_detail']);
    Route::post('make_salary_slip', [SalaryController::class, 'make_salary_slip']);
    ...
    Route::get('/account/export-bank-details', [EmployeeController::class, 'exportBankDetails'])->name('exportBankDetails');

// salary slip automated routes
Route::group(['middleware' => 'auth',], function () {
    
    Route::get('create_slip', [SalaryController::class, 'create_slip'])->name('create_slip');
    Route::post('generate_slip', [SalaryController::class, 'generate'])->name('generate_slip');
    Route::get('show_slip', [SalaryController::class, 'show_slip'])->name('show_slip');
    Route::get('view_slip/{id}', [SalaryController::class, 'view_slip'])->name('view_slip');
    
    // edit feature
    Route::get('/salary-slip/edit/{id}', [SalaryController::class, 'edit_slip'])->name('salarySlip.edit');
    Route::post('/salary-slip/update/{id}', [SalaryController::class, 'update_slip'])->name('salarySlip.update');



    Route::get('new_page', [SalaryController::class, 'new_page'])->name('new_page');
    
    // function to uplad salary ctc data 
    Route::get('upload_salary_sheet', [SalaryController::class, 'upload_salary_sheet'])->name('upload_salary_sheet'); 
    Route::post('salary_sheet_store', [SalaryController::class, 'upload_salary_sheet_store']);
    
    Route::get('/view-letter/{period}', [LoiController::class, 'viewContractLetter']);
 ...
 Route::group(['middleware' => 'auth', 'prefix' => 'account'], function () {
     ...
     // function to uplad sheet monthly 
    Route::get( '/upload_monthly_sheet', [SalarySheetUpload::class, 'upload_salary_sheet']); 
    
    Route::post('upload_monthly_sheet', [SalarySheetUpload::class, 'uploadMonthlySheet']);
    
    Route::post('process_data', [SalarySheetUpload::class, 'process_data']);
    
    
    
    // letter section 
    Route::get('/myletters', [LoiController::class, 'letters']);
    Route::get('/viewMyLoi', [LoiController::class, 'viewMyLoi']);
    Route::get('/downloadMyLoi', [LoiController::class, 'downloadMyLoi']);
    
    // Offer letter section 
    Route::get('/ViewMyOl', [LoiController::class, 'ViewMyOl']);
    Route::get('/downloadViewMyOl', [LoiController::class, 'downloadViewMyOl']);
    Route::get('/viewMyLoi', [LoiController::class, 'viewMyLoi']);
    Route::get('/viewMyExp', [LoiController::class, 'viewMyExp']);
    Route::get('/viewExp/{user_id}', [LoiController::class, 'viewExp']);
    Route::get('/viewLoi/{user_id}', [LoiController::class, 'viewLoi']);
    
    Route::get('/upload_oldata', [LoiController::class, 'upload_oldata']);
    Route::post('/upload_oldata', [LoiController::class, 'submit_oldata']);
    
    // /acknowledgment letter 
    Route::get('ViewMyack', [LoiController::class, 'ViewMyack']);
    
    
     // pt contr
     
     
    Route::get('/skills/employee', [PtController::class, 'index_emp']);  // employee
    Route::get('/skills/employee/{id}', [PtController::class, 'pt_task_edit_emp']);// employee
    Route::post('/skills/employee/{id}', [PtController::class, 'pt_task_edit_emp_store']); // emp
    Route::get('/skills/create', [PtController::class, 'create'])->name('skills.create');  // hr admin
    
    Route::get('/skills/tracker', [PtController::class, 'index'])->name('skills.index'); // hr and super admin
    Route::get('/skills/hr/{id}', [PtController::class, 'pt_task_edit_hr']);// hr
    Route::post('/skills/hr/{id}', [PtController::class, 'pt_task_edit_hr_store']); // hr
    Route::get('/skills/hr/delete/{id}', [PtController::class, 'pt_task_delete']); // hr
    Route::post('/skills', [PtController::class, 'store'])->name('skills.store');
...
Route::group(['middleware' => 'auth', 'prefix' => 'account'], function () {
    ...
    Route::get('dashboard/agent', [DashboardController::class, 'agent_dash'])->name('agent.dash');
    Route::get('dashboard/tl', [DashboardController::class, 'tl_dash'])->name('tl.dash');
    Route::get('dashboard/non-csa', [DashboardController::class, 'non_csa_dash'])->name('non.csa.dash');
    Route::get('/employee-detail/{userId}', [DashboardController::class, 'employee_detail'])->name('employee.detail');
    ...
    Route::get('branches/branch-hierarchy', [BranchController::class, 'hierarchyData'])->name('branch.hierarchy');
    Route::post('branches/changeParent', [BranchController::class, 'changeParent'])->name('branch.changeParent');
    Route::post('branches/search-filter', [BranchController::class, 'searchFilter'])->name('branch.srchFilter');
    Route::post('branches/apply-quick-action', [BranchController::class, 'applyQuickAction'])->name('branches.apply_quick_action');
    ...
    Route::get('my_team/{id}', [DepartmentController::class, 'my_team'])->name('departments.d_head_view');
    ...
    Route::get('attendances/export-all-attendance/{year}/{month}/{id}/{late}/{department}/{designation}/{name}/{branch}', [AttendanceController::class, 'exportAllAttendance'])->name('attendances.export_all_attendance');
    ...
    Route::group(['prefix' => 'shifts'], function () {
        Route::get('import_shift', [EmployeeShiftScheduleController::class, 'import_shift'])->name('shifts-change.import_shift');
        Route::post('import_shift', [EmployeeShiftScheduleController::class, 'import_shift_save'])->name('shifts-change.import_shift_save');
