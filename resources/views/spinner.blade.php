...
.spin-animation {
  animation: spin 5s linear infinite; /* 5s duration for one rotation, linear timing function, infinite loop */
}
...
@media (max-width: 576px) {
  .mainbox {
    ...
    height: 50%;
      ...
      <button type="button" class="btn btn-primary start-button" id="close-modal">Start</button>
...
// Open the modal when the user clicks the start button
...
// Bentuk acak
  ...
  let employees = {!! json_encode($data->pluck('name')) !!};
  let selectedEmployee = employees[Hasil[0] % employees.length];
   
   let rotationAngle = 360 - (Hasil[0] % employees.length) * (360 / employees.length);
   
   const numEmployees = {!! json_encode(count($data)) !!}; // Get the total number of employees from Blade
  const winnerIndex = Math.floor(Math.random() * numEmployees); // Randomly select a winner index

  // Disable button during the spin
  document.querySelector('.spin').disabled = true;

  // Rotate the spinner with the spin-animation class
//   const box = document.getElementById('box');
...
//   if (Rahul.includes(Hasil[0])) SelectedItem = "Rahul";
  ...
  box.style.transform = "rotate(" + (rotationAngle + 360 * 5) + "deg)";
